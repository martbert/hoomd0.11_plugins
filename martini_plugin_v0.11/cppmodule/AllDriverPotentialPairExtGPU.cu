...
cudaError_t gpu_compute_ljm_forces(const pair_args_t& pair_args,
                                   const float4 *d_params)
    {
    return gpu_compute_pair_forces<EvaluatorPairLJM>(pair_args, d_params);
    }

cudaError_t gpu_compute_coulombm_forces(const pair_args_t& pair_args,
                                   const float4 *d_params)
    {
    return gpu_compute_pair_forces<EvaluatorPairCoulombM>(pair_args, d_params);
    }
